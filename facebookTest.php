...
FROM activitylogs
WHERE `Name_App` LIKE \"com.facebook.katana\" AND `Name_Activity` LIKE \"$activity\" AND `activitylogs`.`ID_User`= \"$user\" 
GROUP BY `MyTime`
...
if ($length == 0) {
    ...
    echo "<p>Please <a href=\"index.php\">go back</a> and try another activity</p>";
} else {
    echo "<p>Thanks for trying X-Mind Graphs! Hope it has been useful for you.</p>";
    echo "<p>Feel free to <a href=\"index.php\">go back</a> and try another graph</p>";
}
?>
    </body>
